...
for j in range(len(filter)):
    ...
    t1 = ascii.read('../../results/Ceph_res_nohm'+filter[j]+'.csv')
    t2 = ascii.read('../../results/Ceph_res_nohmH.csv')
    tab = join(t1,t2,keys='sn')
    
    w =np.where((tab['sample_1']=='CSPI')& (tab['cal_1']=='none'))
    w = np.where(tab['cal_1']=='none')
    mass = tab['m_1'][w]
    
    #sys.exit()
    
    ml =  tab['m_1'][w]-tab['ml_1'][w]
    mu = tab['mu_1'][w]- tab['m_1'][w]
    em = (ml+mu)/2.
    res=tab['res_1'][w]
    eres=tab['eres_1'][w]
