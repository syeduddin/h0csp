...
f1  = pd.read_csv('../../data/hosts/CSPHostMass.csv',delimiter=',')
d1=pd.DataFrame(f1)
m = d1['m']
eml = d1['m']-d1['ml']
emu = d1['mu']-d1['m']
...
for j in range(len(path)):
    ...
    for n,i in enumerate(mass):
        if i==11.5: mass[n]=random.uniform(7.2,7.5)
    ...
    print '& $'+path[j][-1:]+'$', '&%.3f'%np.mean(lm.chain['beta']),'(','%.3f'%(np.std(lm.chain['beta'])),')&', '%.3f'%(mean_x1_high-mean_x1_low),'(', '%.3f'%(np.sqrt((error_x1_low**2)+(error_x1_high**2))),')','&','%.2f'%np.median(mass) 
...
pl.savefig('../../plots/AllMassCorr_old.pdf',bbox_inches='tight', dpi=100)
