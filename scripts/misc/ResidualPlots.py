...
file =[ '../../results/'+band+'_trgb_result.txt']
...
for i in range(len(file)):
    ...
    tab = ascii.read('../../data/working/'+band+'_ceph.csv')
    #w= np.where((tab['subtype']!='Ia-91T') & (tab['subtype']!='Ia-91bg') & (tab['caltype']=='none')& (tab['sample']!='bla'))
   
    w = np.where((tab['sn']!='CSP14abk') &  (tab['sn']!='PTF13dyt') &  (tab['sn']!='PTF13dym')& (tab['sn']!='PS1-13eao')& (tab['sn']!='PTF14yw') & (tab['subtype']!='Ia-SC') & (tab['subtype']!='Ia-02cx') & (tab['dist']<0))
    #  
    st = tab['st'][w]
    est = tab['est'][w]
    zhel = tab['zhel'][w]
    ...
    #mu_model = 5.0*np.log10(c*zcmb/72.0) +25
...
t = ascii.read('../../data/lc/Spreadsheet.csv')
#pl.hist(t['res'],histtype='step',lw=2,label='Spreadsheet',bins=20)
#pl.legend(numpoints =1,fontsize=12,loc='upper left')

pl.axvline(0,color='k',lw=3)
pl.tight_layout()
pl.savefig('../../plots/histres_'+band+'_trgb.pdf')
#pl.show()
