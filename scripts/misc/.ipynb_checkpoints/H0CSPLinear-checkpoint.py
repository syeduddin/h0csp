...
w0 = ((tab['sn']!='CSP14abk') &  (tab['sn']!='PTF13dyt') &  (tab['sn']!='PTF13dym') &  (tab['sn']!='PS1-13eao') & (tab['dist']<0) & (tab['zcmb']>0.0))


f1 =open('../../results/'+file[:-4]+'_result_offset_z00.txt','w')
...
def like(par):
    ...
    if  -25.0<p<14.0  and -10.0<p1<10.0 and -10.0<p2<10.0 and 0.<rv<10.0 and -1.<alpha<1. and 0.<sig<1. and 0.<vel<1000. and  0< h0 < 1000.0: # priors
        ...
        mu_sn = mmax[w0] - p - p1*(st[w0] - 1.) - rv*bv[w0] - alpha*(m_csp[w0]-np.median(m_csp[w0]))+(alpha*em[w0])**2

        mu_cal = mmax[w1] - p - p1*(st[w1] - 1.)  - rv*bv[w1] - alpha*(m_csp[w1]-np.median(m_csp[w1]))+(alpha*em[w1])**2
