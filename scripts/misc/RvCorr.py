...
for j in range(len(path)):
    ...
    for i in range(len(sn)):
        ...
        res.append(d2['Rv'].values[i])
        eres.append(d2['eRv'].values[i])
    ...
    pl.xlim(7,12),pl.ylim(0,5.0)
    pl.xlabel(r'$Log \ host \ M_{\rm stellar} \ (M_{\odot})$',fontsize=18)
    pl.ylabel(r'$R_V ('+path[j][-1:]+')$' ,fontsize=18)
...
pl.savefig('plots/RvCorr.pdf',bbox_inches='tight', dpi=100)
