...
st = (tab['st'][w]*1.04)-0.05
est = tab['est'][w]
zhel = tab['zhel'][w]
zcmb = tab['zcmb'][w]
mmax = (tab['Mmax'][w]*0.98) +0.24
emmax = tab['eMmax'][w]
bv = (tab['BV'][w]*0.8) +0.04
ebv = tab['eBV'][w]
m_csp = tab['m'][w]
...
# Liklihood function
def like(par):
    p,p1,rv,alpha,sig,vel,h0 = par
    if  -25.0<p<14.0  and -10.0<p1<10.0 and 0.<rv<10.0 and -1.<alpha<1. and 0.<sig<1. and 0.<vel<1000. and  0< h0 < 1000.0: # priors
...
p0 = np.array([p00,p10,rv0,alpha0,sig0,vel0,h00]).T
...
labels = ["$M_B$","P1", r"$\beta$",r"$\alpha$", r"$\sigma_{int}$",r"$V_{pec}$", r"$H_0$"]
for j in range(ndim):
    ax = axes[j]
...
samples = sampler.chain[:, burnin:, :].reshape((-1, ndim))
 ...
 # Printing results
p0_mcmc,p1_mcmc,rv_mcmc,alpha_mcmc,sig_mcmc,vel_mcmc, H0_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
...
""".format(p0_mcmc, p1_mcmc, rv_mcmc,alpha_mcmc,sig_mcmc,vel_mcmc, H0_mcmc))



# Triangle plot
figure = corner.corner(samples,labels=["$P0$","$P1$", r"$\beta$",r"$\alpha$", r"$\sigma_{int}$","$V_{pec}$", r"$H_0$"],quantiles=[0.16, 0.5, 0.84],truths=[p0_mcmc[0],p1_mcmc[0],rv_mcmc[0],alpha_mcmc[0],sig_mcmc[0],vel_mcmc[0],H0_mcmc[0]],show_titles=True)

#figure.savefig("plots/mcmcH0_"+filter+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")
figure.savefig("../plots/mcmcH0_nop2_"+file[:-4]+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")
