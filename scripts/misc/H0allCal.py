...
def like(par):
    ...
    if  -25.0<p<14.0  and -10.0<p1<10.0 and -10.0<p2<10.0 and 0.<rv<10.0 and -1.<alpha<1. and 0.<sig<1. and 0.<vel<1000. and  0< h0 < 1000.0 and 0<sd_cal<10 and 0<b<10:  # priors
        ...
        lnlike2= np.sum(np.log(multivariate_normal.pdf(dist[w2], mean=mu_obscal, cov=covmat(p1,p2,rv,sd_cal)))) + invgamma.logpdf(sd_cal**2, a=2., scale=b)
...
# Chains
fig, axes = pl.subplots(10, figsize=(10, 7), sharex=True)
samples = sampler.get_chain()
labels = ["$M_B$","P1", "P2", r"$\beta$",r"$\alpha$", r"$\sigma_{int}$",r"$V_{pec}$", r"$H_0$",r"$\sigma_{cal}$", "b"]
...
fig.savefig("steps_sd"+filter+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")

samples = sampler.chain[:, burnin:, :].reshape((-1, ndim))


 # Printing results
p0_mcmc,p1_mcmc,p2_mcmc,rv_mcmc,alpha_mcmc,sig_mcmc,vel_mcmc, H0_mcmc, sd_cal_mcmc,b_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
...
""".format(p0_mcmc, p1_mcmc, p2_mcmc,rv_mcmc,alpha_mcmc,sig_mcmc,vel_mcmc, H0_mcmc, sd_cal_mcmc,b_mcmc))
...
# Triangle plot
figure = triangle.corner(samples,labels=["$P0$","$P1$", "$P2$", r"$\beta$",r"$\alpha$", r"$\sigma_{int}$","$V_{pec}$", r"$H_0$",r"$\sigma_{cal}$", "$b$" ],quantiles=[0.16, 0.5, 0.84],truths=[p0_mcmc[0],p1_mcmc[0],p2_mcmc[0],rv_mcmc[0],alpha_mcmc[0],sig_mcmc[0],vel_mcmc[0],H0_mcmc[0], sd_cal_mcmc[0],b_mcmc[0]],show_titles=True)

#figure.savefig("plots/mcmcH0_"+filter+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")
figure.savefig("mcmcH0_sd"+filter+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")
