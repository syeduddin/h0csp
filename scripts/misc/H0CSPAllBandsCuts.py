...
for i in range(len(filter)):
    ...
    ebv = tab['eBV_2']
    m_csp = tab['m_2']
    eml = (tab['m_2']-tab['ml_2'])
    emu = (tab['mu_2']-tab['m_2'])
    em = (emu+eml)/2.
    dist = tab['dist_2']
    edist = tab['edist_2']
    c_ms = tab['covMs_2']
    ...
    w0 = np.where((tab['sn']!='CSP14abk') &  (tab['sn']!='PTF13dyt') &  (tab['sn']!='PTF13dym') &  (tab['sn']!='PS1-13eao')& (tab['zcmb_2']>0.01) &(tab['dist_2']<0)  & (tab['t0_2']<5) & (tab['st_2']> 0.5) & (tab['BV_2']<0.5))
    w1 = (dist>0)
...
""".format(p0_mcmc, p1_mcmc, p2_mcmc,rv_mcmc,alpha_mcmc,sig_mcmc,vel_mcmc, H0_mcmc))
    ...
    f1 =open('../../results/TRGB_result_allcut_'+filter[i]+'.txt','w')
