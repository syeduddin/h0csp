...
vel = ascii.read('/Users/suddin/Dropbox/Scripts/ExpVel/data/vels_all.dat',format='basic')
...
vel.rename_column('ID','sn')      # for consistency

lc = ascii.read('../data/working/'+file)

tab = join(lc,vel,join_type='outer',keys='sn')
...
cal = ascii.read('../data/working/'+file)
...
def like(par):
    ...
    if  -25.0<p<14.0  and -10.0<p1<10.0 and -10.0<p2<10.0 and 0.<rv<10.0 and -1.<alpha<1. and 0.<sig<1. and 0.<vel<1000. and  0< h0 < 1000.0: # priors
        ...
        mu_cal = mmax1[w1] - p - p1*(st1[w1] - 1.) -  p2*(st1[w1] - 1.)**2 - rv*bv1[w1] - alpha*(m_csp1[w1]-np.median(m_csp1[w1]))
