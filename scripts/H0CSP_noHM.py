...
# Liklihood function
def like(par):
    p,p1,p2,rv,sig,vel,h0 = par
    if  -25.0<p<14.0  and -10.0<p1<10.0 and -10.0<p2<10.0 and 0.<rv<10.0 and 0.<sig<1. and 0.<vel<1000. and  0< h0 < 1000.0: # priors
        

        
        mu_obs = mmax - p - p1*(st - 1.) -  p2*(st - 1.)**2 - rv*bv # no host mass correction
...
p0 = np.array([p00,p10,p20,rv0,sig0,vel0,h00]).T
...
labels = ["$M_B$","P1", "P2",r"$\alpha$", r"$\sigma_{int}$",r"$V_{pec}$", r"$H_0$"]
for j in range(ndim):
    ax = axes[j]
...
#print(tau)
 # Printing results
p0_mcmc,p1_mcmc,p2_mcmc,rv_mcmc,sig_mcmc,vel_mcmc, H0_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
...
""".format(p0_mcmc, p1_mcmc, p2_mcmc,rv_mcmc,sig_mcmc,vel_mcmc, H0_mcmc))



f1.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%('p0','p1','p2','beta','sig_int','vel','H0'))

f1.write('%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n'%(p0_mcmc[0],p1_mcmc[0],p2_mcmc[0],rv_mcmc[0],sig_mcmc[0],vel_mcmc[0],H0_mcmc[0]))

f1.write('%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n'%(p0_mcmc[1],p1_mcmc[1],p2_mcmc[1],rv_mcmc[1],sig_mcmc[1],vel_mcmc[1],H0_mcmc[1]))
f1.write('%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n'%(p0_mcmc[2],p1_mcmc[2],p2_mcmc[2],rv_mcmc[2],sig_mcmc[2],vel_mcmc[2],H0_mcmc[2]))
