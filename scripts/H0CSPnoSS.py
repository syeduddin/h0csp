...
t = ascii.read('../data/working/'+file)
#print (len(t))
ew = ascii.read('../data/working/branch_type_ceph.csv')
print (Counter(ew['br']))

ew = unique(ew,keys='sn',keep='last')
w0 = np.where(ew['br']=='CN')
ew=ew[w0]
print (len(ew))
tab = join(t,ew,keys='sn')
...
c1 = np.where(tab['sample'][w]=='CSPI')
s2 = np.where(tab['sample'][w]=='CSPII')
...
f1 =open('../results/'+file[:-4]+'_result_CN.txt','w') # check file name
...
figure.savefig("/Users/suddin/Science/SNeSurveys/CSP/mcmcplots/mcmcH091_"+file[:-4]+"_"+str(nwalkers)+"_"+str(ssize)+".pdf")
