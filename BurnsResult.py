...
#tab = ascii.read('data/Bmax+Mass.dat')
...
def like(par):
    ...
    if  -25.0<p<-15.0  and -10.0<p1<10.0 and -10.0<p2<10.0 and 0.<rv<10.0 and -1.<alpha<1. and 0.<sig<1. and 0.<vel<1000.:
        ...
        mu_stat = (fac*est)**2 +emmax**2 +(rv*ebv)**2+2*fac*c_ms+rv*c_mbv+sig**2+(0.00000723*vel/zcmb)**2
...
# Triangle plot
figure = triangle.corner(samples,labels=["$P0$","$P1$", "$P2$", "$R_V$",r"$\alpha$", r"$\sigma_{int}$","$V_{pec}$"],quantiles=[0.16, 0.5, 0.84],truths=[p0_mcmc[0],p1_mcmc[0],p2_mcmc[0],rv_mcmc[0],alpha_mcmc[0],sig_mcmc[0],vel_mcmc[0]],show_titles=True)
